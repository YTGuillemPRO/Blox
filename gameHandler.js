...
class SellLemonsGame {
    ...
    build() {
        this.buildTree(); this.buildStand(); this.buildDecorations(); this.spawnLemons(); this.buildHud(); this.start();
    }
    ...
    buildTree() {
        ...
        for (const [x, y, z, r] of leafData) {
            ...
            leaf.position.set(x, y, z); leaf.castShadow = true; tree.add(leaf);
        }
    ...
    buildDecorations() {
        ...
        for (const [x, y, z, type] of decor) {
            ...
            mesh.position.set(x, y + (type === 'rock' ? 0.3 : 0.5), z); mesh.castShadow = true; this.add(mesh);
        }
        const sun = new THREE.Mesh(new THREE.SphereGeometry(8, 16, 16), new THREE.MeshBasicMaterial({ color: 0xfff176 }));
        sun.position.set(60, 80, -60); this.add(sun);
        ...
        for (const [x, y, z] of clouds) {
            ...
            for (const [cx, cy, cz, r] of parts) {
                ...
                sphere.position.set(cx, cy, cz); cloud.add(sphere);
            }
            cloud.position.set(x, y, z); this.add(cloud);
        }
    }
    ...
    spawnLemons() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            lemon.position.set(x, y, z); lemon.castShadow = true; lemon.userData.isLemon = true; lemon.userData.collected = false;
            this.add(lemon); this.lemons.push(lemon);
        }
    }
    ...
    onClick(worldPoint) {
        ...
        let picked = false; let nearest = null; let nearestDist = Infinity;
        for (const lemon of this.lemons) {
            if (lemon.userData.collected) continue;
            const dx = lemon.position.x - px.x; const dy = lemon.position.y - px.y; const dz = lemon.position.z - px.z;
            const d = Math.sqrt(dx*dx + dy*dy + dz*dz);
            if (d < this.pickupRadius && d < nearestDist) { nearest = lemon; nearestDist = d; }
        }
        if (nearest) {
            nearest.userData.collected = true; this.scene.remove(nearest); this.inventory++;
            this.showToast(`+1 🍋`); picked = true; this.updateHud();
        }
        return picked;
    }
    ...
    sellInventory() {
        ...
        const earned = this.inventory * this.moneyPerLemon; this.money += earned; this.inventory = 0;
        this.showToast(`+$${earned} 💰`); this.updateHud();
    }
    ...
    buyUpgrade(upgradeId) {
        ...
        if (upgradeId === 'tree2' && this.treeLevel < 2) { this.money -= cost; this.treeLevel = 2; this.respawnLemons(); return true; }
        if (upgradeId === 'tree3' && this.treeLevel < 3) { this.money -= cost; this.treeLevel = 3; this.respawnLemons(); return true; }
        if (upgradeId === 'autoSeller' && !this.autoSeller) { this.money -= cost; this.autoSeller = true; this.startAutoSeller(); return true; }
        return false;
    }

    respawnLemons() {
        for (const lemon of this.lemons) this.scene.remove(lemon);
        this.lemons = []; this.spawnLemons();
    }
    ...
    startAutoSeller() {
        ...
        this.autoSellInterval = setInterval(() => {
            if (this.inventory > 0) {
                const earned = this.inventory * this.moneyPerLemon; this.money += earned; this.inventory = 0;
                this.showToast(`Auto-Venta: +$${earned} 🤖`); this.updateHud();
            }
        }, 4000);
    }
    ...
    update() {
        ...
        if (sellBtn) {
            ...
            if (shouldShow) sellBtn.textContent = `💰 VENDER ${this.inventory} x $${this.moneyPerLemon} = +$${this.inventory * this.moneyPerLemon}`;
        }
    ...
    buildHud() {
        ...
        const moneyEl = document.createElement('div'); moneyEl.className = 'hud-lemon-money'; moneyEl.id = 'lemon-money'; moneyEl.textContent = '$0'; slot.appendChild(moneyEl);
        const infoEl = document.createElement('div'); infoEl.className = 'hud-lemon-info'; infoEl.id = 'lemon-info'; infoEl.innerHTML = '🍋 <span id="lemon-inv">0</span> &nbsp; 🌳 Nivel <span id="lemon-tree">1</span>'; slot.appendChild(infoEl);
        const sellBtn = document.createElement('button'); sellBtn.className = 'hud-lemon-sell-btn'; sellBtn.id = 'lemon-sell-btn'; sellBtn.textContent = '💰 VENDER'; slot.appendChild(sellBtn);
        const shopBtn = document.createElement('button'); shopBtn.className = 'hud-lemon-shop-btn'; shopBtn.id = 'lemon-shop-btn'; shopBtn.textContent = '🛒 Tienda'; slot.appendChild(shopBtn);
        const modal = document.createElement('div'); modal.className = 'lemon-shop-modal'; modal.id = 'lemon-shop-modal';
        modal.innerHTML = `<div class="lemon-shop-box"><h2>🛒 Tienda</h2><p class="shop-money">💰 $<span id="shop-money-amount">0</span></p><div class="upgrade"><strong>🌳 Árbol Nivel 2</strong><p>14 limones por cosecha.</p><button class="upgrade-btn" data-id="tree2">$100</button></div><div class="upgrade"><strong>🌳 Árbol Nivel 3</strong><p>20 limones por cosecha.</p><button class="upgrade-btn" data-id="tree3">$350</button></div><div class="upgrade"><strong>🤖 Vendedor Automático</strong><p>Vende tus limones cada 4s.</p><button class="upgrade-btn" data-id="autoSeller">$250</button></div><button class="close-shop" id="close-shop">← Volver al juego</button></div>`;
        slot.appendChild(modal);
        const toast = document.createElement('div'); toast.className = 'lemon-toast'; toast.id = 'lemon-toast'; document.body.appendChild(toast);

        sellBtn.addEventListener('click', () => this.sellInventory());
        shopBtn.addEventListener('click', () => { modal.classList.add('open'); document.getElementById('shop-money-amount').textContent = this.money; this.refreshShopButtons(); });
        ...
        slot.querySelectorAll('.upgrade-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const id = btn.dataset.id; const ok = this.buyUpgrade(id);
                if (ok) { btn.disabled = true; btn.classList.add('owned'); btn.textContent = '✓ Comprado'; document.getElementById('shop-money-amount').textContent = this.money; this.refreshShopButtons(); this.updateHud(); }
                else { btn.textContent = '✗ Sin dinero'; setTimeout(() => { this.refreshShopButtons(); }, 800); }
            });
        });
        ...
        this.onCanvasClick = (e) => {
            ...
            const ray = new THREE.Raycaster(); ray.setFromCamera(mouse, this.handler.camera);
            const hits = ray.intersectObjects(this.lemons.filter(l => !l.userData.collected));
            if (hits.length > 0) this.onClick(hits[0].object.position);
        };
    ...
    refreshShopButtons() {
        document.querySelectorAll('.upgrade-btn').forEach(btn => {
            const id = btn.dataset.id; let bought = false;
            if (id === 'tree2') bought = this.treeLevel >= 2; if (id === 'tree3') bought = this.treeLevel >= 3; if (id === 'autoSeller') bought = this.autoSeller;
            if (bought) { btn.classList.add('owned'); btn.disabled = true; btn.textContent = '✓ Comprado'; }
            else { btn.classList.remove('owned'); btn.disabled = false; btn.textContent = id === 'tree2' ? '$100' : id === 'tree3' ? '$350' : '$250'; }
        });
    }

    updateHud() {
        const moneyEl = document.getElementById('lemon-money'); const invEl = document.getElementById('lemon-inv'); const treeEl = document.getElementById('lemon-tree'); const shopMoney = document.getElementById('shop-money-amount');
        if (moneyEl) moneyEl.textContent = `$${this.money}`; if (invEl) invEl.textContent = this.inventory; if (treeEl) treeEl.textContent = this.treeLevel;
        if (shopMoney && document.getElementById('lemon-shop-modal').classList.contains('open')) shopMoney.textContent = this.money;
    }

    showToast(text) {
        const toast = document.getElementById('lemon-toast'); if (!toast) return;
        toast.textContent = text; toast.classList.add('show'); clearTimeout(this._toastT);
        this._toastT = setTimeout(() => toast.classList.remove('show'), 1500);
    }

    cleanup() {
        for (const obj of this.objects) this.scene.remove(obj);
        this.objects = []; this.lemons = [];
        if (this.handler.renderer?.domElement && this.onCanvasClick) this.handler.renderer.domElement.removeEventListener('click', this.onCanvasClick);
        this.stopAutoSeller();
        const slot = document.getElementById('game-specific-hud'); if (slot) slot.innerHTML = '';
        const toast = document.getElementById('lemon-toast'); if (toast) toast.remove();
    }
}
...
class GameHandler {
    ...
    createCastle(x, z, teamColor) {
        ...
        positions.forEach(p => {
            ...
            tower.position.set(p[0], 10, p[1]); tower.castShadow = true; tower.receiveShadow = true; castle.add(tower);
            for(let i = -3; i <= 3; i += 2) {
                const cren = new THREE.Mesh(new THREE.BoxGeometry(2, 2, 2), stoneMat); cren.position.set(p[0] + i, 21, p[1] + 3.5); castle.add(cren);
                const cren2 = new THREE.Mesh(new THREE.BoxGeometry(2, 2, 2), stoneMat); cren2.position.set(p[0] + 3.5, 21, p[1] + i); castle.add(cren2);
                const cren3 = new THREE.Mesh(new THREE.BoxGeometry(2, 2, 2), stoneMat); cren3.position.set(p[0] + i, 21, p[1] - 3.5); castle.add(cren3);
                const cren4 = new THREE.Mesh(new THREE.BoxGeometry(2, 2, 2), stoneMat); cren4.position.set(p[0] - 3.5, 21, p[1] + i); castle.add(cren4);
            }
    ...
    startGameMode(mode) {
        ...
        if (mode === 'pvp') {
            ...
            this.activeGame = new SellLemonsGame(this);
            this.activeGame.build();
        }
    }
    ...
    updateRemotePlayer(id, data) {
        ...
        if (data.chat && data.chat !== "") {
            p.label.element.innerHTML = data.chat; p.label.element.style.opacity = '1';
            clearTimeout(p.chatTimeout); p.chatTimeout = setTimeout(() => { p.label.element.style.opacity = '0'; }, 4000);
        }
    }

    removeRemotePlayer(id) {
        if (this.otherPlayers[id]) { this.scene.remove(this.otherPlayers[id].mesh); delete this.otherPlayers[id]; }
    }
